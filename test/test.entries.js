...
tape( 'attached to the prototype of the main export is an `entries` method for returning an iterator for iterating over array key-value pairs', function test( t ) {
	t.strictEqual( hasOwnProp( Int64Array.prototype, 'entries' ), true, 'has property' );
	t.strictEqual( isFunction( Int64Array.prototype.entries ), true, 'has method' );
	t.end();
});

tape( 'the method throws an error if invoked with a `this` context which is not a 64-bit signed integer instance', function test( t ) {
	var values;
	var arr;
	var i;
...
tape( 'the method returns an iterator protocol-compliant object', function test( t ) {
	...
	for ( i = 0; i < arr.length; i++ ) {
		...
		t.strictEqual( isInt64( v.value[ 1 ] ), true, 'returns a 64-bit signed integer' );
