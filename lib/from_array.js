...
* @param {Array} arr - array containing 64-bit signed integers
	...
	for ( i = 0; i < len; i++ ) {
		...
		if ( !isInt64( v ) ) {
			try {
				...
				return new TypeError( format( 'invalid argument. An array element must be either a 64-bit signed integer or a value which can be converted to a 64-bit signed integer. Value: `%s`.', v ) );
			}
		}
		toWords( v, words, 1, 0 );
