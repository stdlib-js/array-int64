	...
	while ( true ) {
		...
		if ( !isInt64( u ) ) {
			try {
				...
				return new TypeError( format( 'invalid argument. An iterator must return either a 64-bit signed integer or a value which can be converted to a 64-bit signed integer. Value: `%s`.', u ) );
			}
		}
		toWords( u, words, 1, 0 );
